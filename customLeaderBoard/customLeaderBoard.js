...
static get observedAttributes() {
    return ["actualuser", "tag", "redirect"];
}
...
attributeChangedCallback(attribute, oldValue, newValue) {
    if (oldValue !== newValue) {
        switch (attribute) {
            case "actualuser":
                console.log("actualuser", newValue);
                this.actualuser = newValue;
                break;
            case "tag":
...
fetchMeta() {
  delay(1000).then(() => {
    fetch(`${this.url}/v2/users?items_per_page=200`, requestOptions)
      .then(response => response.json())
      .then(metaData => {
        this.pages = metaData.meta.totalPages;
        this.fetchAlumn();
...
fetchAlumn() {
  ...
  delay(1000).then(() => {
    for (let i = 0; i < pages; i++) {
      fetchPromises.push(
        fetch(`${url}/v2/users?items_per_page=200&page=${i}`, this.requestOptions)
          .then(response => response.json())
          .then(data => {
    ...
    for (let i = 0; i < usersByTag.length; i++) {
      fetchPromises.push(
        fetch(`${url}/v2/users/${usersByTag[i].id}/progress`, requestOptions)
          .then(response => response.json())
          .then(progressData => {
            const isDuplicate = filteredUsers.some(user => user.userID === usersByTag[i].id);
            if (!isDuplicate) {
